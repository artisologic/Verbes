'use strict';

var data = {

	persons: ['je', 'tu', 'il/elle/on', 'nous', 'vous', 'ils/elles'],
	verbs: [
		{
			infinitiveUk: 'to be',
			infinitiveFr: 'être',
			conjugations: ['suis', 'es', 'est', 'sommes', 'êtes', 'sont']
		},

		{
			infinitiveUk: 'to have',
			infinitiveFr: 'avoir',
			conjugations: ['ai', 'as', 'a', 'avons', 'avez', 'ont']
		},

		{
			infinitiveUk: 'to go',
			infinitiveFr: 'aller',
			conjugations: ['vais', 'vas', 'va', 'allons', 'allez', 'vont']
		},

		{
			infinitiveUk: 'to drink',
			infinitiveFr: 'boire',
			conjugations: ['bois', 'bois', 'boit', 'buvons', 'buvez', 'boivent']
		},

		{
			infinitiveUk: 'to sing',
			infinitiveFr: 'chanter',
			conjugations: ['chante', 'chantes', 'chante', 'chantons', 'chantez', 'chantent']
		},

		{
			infinitiveUk: 'to choose',
			infinitiveFr: 'choisir',
			conjugations: ['choisis', 'choisis', 'choisit', 'choisissons', 'choisissez', 'choisissent']
		},

		{
			infinitiveUk: 'to know',
			infinitiveFr: 'connaître',
			conjugations: ['connais', 'connais', 'connaît', 'connaissons', 'connaissez', 'connaissent']
		},

		{
			infinitiveUk: 'must',
			infinitiveFr: 'devoir',
			conjugations: ['dois', 'dois', 'doit', 'devons', 'devez', 'doivent']
		},

		{
			infinitiveUk: 'to write',
			infinitiveFr: 'écrire',
			conjugations: ['écris', 'écris', 'écrit', 'écrivons', 'érivez', 'écrivent']
		},

		{
			infinitiveUk: 'to do',
			infinitiveFr: 'faire',
			conjugations: ['fais', 'fais', 'fait', 'faisons', 'faites', 'font']
		},

		{
			infinitiveUk: 'can',
			infinitiveFr: 'pouvoir',
			conjugations: ['peux', 'peux', 'peut', 'pouvons', 'pouvez', 'peuvent']
		},

		{
			infinitiveUk: 'to take',
			infinitiveFr: 'prendre',
			conjugations: ['prends', 'prends', 'prend', 'prenons', 'prenez', 'prennent']
		},

		{
			infinitiveUk: 'to know',
			infinitiveFr: 'savoir',
			conjugations: ['sais', 'sais', 'sait', 'savons', 'savez', 'savent']
		},

		{
			infinitiveUk: 'to come',
			infinitiveFr: 'venir',
			conjugations: ['viens', 'viens', 'vient', 'venons', 'venez', 'viennent']
		},

		{
			infinitiveUk: 'to see',
			infinitiveFr: 'voir',
			conjugations: ['vois', 'vois', 'voit', 'voyons', 'voyez', 'voient']
		},

		{
			infinitiveUk: 'to want',
			infinitiveFr: 'vouloir',
			conjugations: ['veux', 'veux', 'veut', 'voulons', 'voulez', 'veulent']
		},

		{
			infinitiveUk: 'to say',
			infinitiveFr: 'dire',
			conjugations: ['dis', 'dis', 'dit', 'disons', 'dites', 'disent']
		},

		{
			infinitiveUk: 'to read',
			infinitiveFr: 'lire',
			conjugations: ['lis', 'lis', 'lit', 'lisons', 'lisez', 'lisent']
		},

		{
			infinitiveUk: 'to put',
			infinitiveFr: 'mettre',
			conjugations: ['mets', 'mets', 'met', 'mettons', 'mettez', 'mettent']
		},

		{
			infinitiveUk: 'to leave',
			infinitiveFr: 'partir',
			conjugations: ['pars', 'pars', 'part', 'partons', 'partez', 'partent']
		},

		{
			infinitiveUk: 'to receive',
			infinitiveFr: 'recevoir',
			conjugations: ['reçois', 'reçois', 'reçoit', 'recevons', 'recevez', 'reçoivent']
		}
	]

};

data.nbVerbs = data.verbs.length;
data.nbPersons = data.persons.length;
